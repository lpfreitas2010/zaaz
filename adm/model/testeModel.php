<?php
	...
	**/
	...
	//CLASSE ****-
	class testeModel extends config_model_adm {
...
//***************************************************************************************************************************************************************
		function inserir(){
			...
			$this->setValue($this->getCampos('campo')); //Valores ****-
			$exec = $this->Create();
			$this->limpo_campos();
			$this->setUltimo_id($this->get_last_id()); //Pego o id criado
			if(count($exec) >= 1){
				return true;
			}else{
				return false;
...
//***************************************************************************************************************************************************************
		function editar(){
			$this->carrego_parametros(); // Carrego parametros
			$this->setTable($this->getTabela()); // Tabela
			$this->setColuna('modificado')->setColuna('campo'); // Colunas ****-
			$this->setValue($this->getModificado())->setValue($this->getCampos('campo')); //Valores ****-
