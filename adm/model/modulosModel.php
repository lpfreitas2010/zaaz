<?php
	...
	**/
	...
	//CLASSE ****-
	class modulosModel extends config_model_adm {
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
			$this->setTabela('adm_usuario_modulo'); // Tabela ****-
		}
...
//***************************************************************************************************************************************************************
		function listagem($limit = null){
			...
			$this->setColuna($tabela.'.id')
				 ...
				 ->setColuna($tabela.'.modulo')
				 ->setColuna($tabela.'.descricao')
				 ;
			...
			if(!empty($query_pesquisa) && $query_pesquisa != "Pesquisa Avançada"){
				...
				if($conf_personalizada == false){
					...
					$array_pesq[] = $this->return_where_like($tabela.'.modulo',$query_pesquisa);
					$array_pesq[] = $this->return_where_like($tabela.'.descricao',$query_pesquisa);
			...
			for ($i=0; $i <count($order_by) ; $i++) {
				...
				if($order_by[$i] == 'Módulo CRESCENTE'){
					$array_orderby[] = $this->return_order_by($tabela.'.modulo','ASC');
				}
				//CAMPO Módulo DECRESCENTE
				if($order_by[$i] == 'Módulo DECRESCENTE'){
					$array_orderby[] = $this->return_order_by($tabela.'.modulo','DESC');
				}
...
//***************************************************************************************************************************************************************
		function inserir(){
			$this->carrego_parametros(); // Carrego parametros
			$this->setTable($this->getTabela()); // Tabela
			$this->setColuna('modulo')->setColuna('descricao'); // Colunas ****-
			$this->setValue($this->getCampos('modulo'))->setValue($this->getCampos('descricao')); //Valores ****-
			$exec = $this->Create();
			$this->limpo_campos();
...
//***************************************************************************************************************************************************************
		function editar(){
			$this->carrego_parametros(); // Carrego parametros
			$this->setTable($this->getTabela()); // Tabela
			$this->setColuna('modulo')->setColuna('descricao')->setColuna('modificado'); // Colunas ****-
			$this->setValue($this->getCampos('modulo'))->setValue($this->getCampos('descricao'))->setValue($this->getModificado()); //Valores ****-
