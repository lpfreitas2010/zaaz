<?php
	...
	**/
	...
	//CLASSE ****-
	class adm_emails_enviadosModel extends config_model_adm {
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
			$this->setTabela('adm_email_enviados'); // Tabela ****-
		}
...
//***************************************************************************************************************************************************************
		function listagem($limit = null){
			...
			//===========================================================
			//MONTO INNER JOIN ****-
			$this->setInner('INNER JOIN adm_usuario ON adm_usuario.id = adm_email_enviados.adm_usuario_id'); // Inner join
			...
			if(!empty($query_pesquisa) && $query_pesquisa != "Pesquisa Avançada"){
				...
				if($conf_personalizada == false){
				    ...
				    $array_pesq[] = $this->return_where_char('adm_usuario.nome',"=",$query_pesquisa);
					$array_pesq[] = $this->return_where_like($tabela.'.mensagem',$query_pesquisa);
					$array_pesq[] = $this->return_where_like($tabela.'.email_re',$query_pesquisa);
...
//***************************************************************************************************************************************************************
		function detalhamento(){
			...
			$this->setColuna($this->getTabela().'.id')
				 ...
				 ->setColuna($this->getTabela().'.email_re')
				 ->setColuna($this->getTabela().'.mensagem')
				 ->setColuna($this->getTabela().'.status')
