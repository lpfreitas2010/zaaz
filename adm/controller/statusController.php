...
class statusController {
	...
	function __construct() {
		...
		$this->core->includeController('status',$this->dir_app); //incluo model ****-
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
            ...
            $this->url_pagina                    = "status"; //Url da página
            $this->nome_pagina_singular          = "Status"; // Nome da página singular
            $this->nome_pagina_plural            = "Status"; // Nome da página plural
            $this->btns_acoes['foco_campo_form'] = "status"; // Foco campo form add e edd
...
//***************************************************************************************************************************************************************
		function monto_conteudo_html_pagina($param){
			...
			if($param == 'campos_html_add_edd'){
				...
				$mont_html->set_array(null,'input_name','status');
				$mont_html->set_array(null,'input_id','status');
				...
				$mont_html->set_array(null,'input_title','Digite o Status');
			...
			if($param == 'listagem'){
				...
				$this->funcoes->set_array(0,'th_class','hidden-md text-center col-md-2 col-lg-1');
				$this->funcoes->set_array(0,'th_outros','');
				$this->funcoes->set_array(1,'th_titulo','Status');
				$this->funcoes->set_array(1,'th_class',' col-md-5 col-lg-9');
				$this->funcoes->set_array(1,'th_outros','');
				$this->funcoes->set_array(2,'th_titulo','<i class="fa fa-calendar-o"></i> Data Modificado');
				$this->funcoes->set_array(2,'th_class','text-center col-md-3 col-lg-2');
...
//***************************************************************************************************************************************************************
	function add_edd(){
        ...
        (string) $status      = $this->funcoes->anti_injection($_POST['status']);

		//CONVERTO STRING PARA MAIUSCULO E MINUSCULO
		$status = $this->funcoes->conv_string($status,2);
		...
		if(empty($id)){
			...
			//===========================================================
			//VALIDO OS DADOS ****-
			if($this->model->retorn_campo_editar('status', $status) == $status){
				$status_u = true;
			}
			$this->funcoes->set('Status',"status", $status)->is_required()->min_length(2)->max_length(250)->is_unique($status_u);
		...
		//*************************************************************************************
			...
			$this->model->setId($id);
			if($this->model->retorn_campo_editar_val_id('status') != $status){
				if($this->model->retorn_campo_editar('status', $status) == $status){  $status_u = true;  }
				$this->funcoes->set('Status',"status", $status)->is_required()->min_length(2)->max_length(250)->is_unique($status_u);
			}else{
