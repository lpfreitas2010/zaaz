...
if (!empty($_GET['cmd']) || !empty($_POST['cmd'])) {
	$control = new modulos_acoesController();  // ****-
}

//CLASS ****-
class modulos_acoesController {
	...
	function __construct() {
		...
		$this->core->includeController('modulos_acoes',$this->dir_app); //incluo model ****-

		//INSTANCIO
		$this->funcoes           = new funcoes();     //Instancio Funções
		$this->model             = new modulos_acoesModel();  //Instancio Model ****-
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
            ...
            $this->url_pagina                    = "modulos_acoes"; //Url da página
            $this->nome_pagina_singular          = "Ação"; // Nome da página singular
            $this->nome_pagina_plural            = "Restrições Ações"; // Nome da página plural
            $this->btns_acoes['foco_campo_form'] = "acoes"; // Foco campo form add e edd
            ...
            $this->btns_acoes['title_campo_pesquisa'] = 'ID ou Ação'; // Parametros de pesquisa
...
//***************************************************************************************************************************************************************
		function monto_conteudo_html_pagina($param){
			...
			if($param == 'campos_html_add_edd'){
				...
				$mont_html->set_array(null,'input_name','acoes');
				$mont_html->set_array(null,'input_id','acoes');
				...
				$acoes = $mont_html->monto_html_input();
				...
				$mont_html->set_array(0,'conteudo'," {$acoes} "); //conteudo ***
			...
			if($param == 'listagem'){
				...
				$this->funcoes->set_array(12,'value',"AÇÃO CRESCENTE");
				$this->funcoes->set_array(13,'value',"AÇÃO DECRESCENTE");
				...
				$this->funcoes->set_array(1,'th_class','col-md-4 col-lg-6');
				...
				$this->funcoes->set_array(3,'th_class','hidden-md text-center col-md-3');
				$this->funcoes->set_array(3,'th_outros','');
				$this->funcoes->set_array(4,'th_titulo','<i class="fa fa-calendar-o"></i> Data Cadastro');
				$this->funcoes->set_array(4,'th_class','hidden-md text-center col-md-3');
				...
				$mont_html->set_array(0,'url',$this->core->get_config('dir_raiz_http').$this->dir_app."/modulos_acoes_default/busca/ID Ação: [bd:id]"); // url de redirecionamento $this->core->get_config('dir_raiz_http').$this->dir_app."/"
				$mont_html->set_array(0,'status_btn_detalhamento',$this->btns_acoes['status_btn_detalhamento']); // Status ação detalhamento true or false
				$mont_html->set_array(0,'status_btn_editar',$this->btns_acoes['status_btn_editar']); // Status ação editar true or false
				$mont_html->set_array(0,'status_btn_ativar',$this->btns_acoes['status_btn_ativar']); // Status ação ativar e desativar true or false
				$mont_html->set_array(0,'url_btn_ativar',$this->cmds['action_ativar']); // Action do btn ativar
				...
				$mont_html->set_array(1,'td',"[bd:acoes]"); // Coluna montada **
				...
				$mont_html->set_array(2,'class',"text-center {$acao_linha2['class']}"); // Classes
				$mont_html->set_array(2,'outros'," {$acao_linha2['outros']}"); //Outros parametros do td
				$mont_html->set_array(3,'td',"{$modificado}"); // Coluna montada **
		...
		//SETO VALORES EM EDITAR
			...
			if(!empty($exec)){
				...
				for ($i=0; $i < count($exec) ; $i++) {
					...
					$this->funcoes->set_array(0,'campo_set_form','acoes');
					$this->funcoes->set_array(0,'valor_campo_set',$exec[$i]['acoes']);
...
//***************************************************************************************************************************************************************
	function add_edd(){
		...
		$acoes = $this->funcoes->anti_injection($_POST['acoes'],'html');

		//CONVERTO STRING PARA MAIUSCULO E MINUSCULO
		//$acoes = $this->funcoes->conv_string($acoes,2);
		...
		if(empty($id)){
			...
			//===========================================================
			//VALIDO OS DADOS ****-
			if($this->model->retorn_campo_editar('acoes', $acoes) == $acoes){
				$acoes_u = true;
			}
      		$this->funcoes->set('Ação',"acoes", $acoes)->is_required()->min_length(4)->max_length(250)->is_unique($acoes_u);

			//===========================================================
	        //SETO OS DADOS ****-
			$this->model->setCampos('acoes',$acoes);
		...
		//*************************************************************************************
			...
			$this->model->setId($id);
			if($this->model->retorn_campo_editar_val_id('acoes') != $acoes){
				if($this->model->retorn_campo_editar('acoes', $acoes) == $acoes){  $acoes_u = true;  }
				$this->funcoes->set('Ação',"acoes", $acoes)->is_required()->min_length(2)->max_length(250)->is_unique($acoes_u);
			}else{
				$this->funcoes->set('Ação',"acoes", $acoes)->is_required()->min_length(2)->max_length(250);
			}
			//===========================================================
			//SETO OS DADOS ****-
			$this->model->setCampos('acoes',$acoes);
