...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
            ...
            $this->url_pagina                    = "cache"; //Url da página
            $this->nome_pagina_singular          = "Cache do Sistema"; // Nome da página singular
            $this->nome_pagina_plural            = "Cache do Sistema"; // Nome da página plural
            $this->btns_acoes['foco_campo_form'] = "opcoes_cache"; // Foco campo form add e edd
			...
			$this->btns_acoes['status_area_ordenacao']         = false; // Status da ordenação [true or false]
			$this->btns_acoes['status_area_paginacao']         = false; // Status de paginação [ true or false ]
			$this->btns_acoes['status_area_total_registros']   = false; // Status de total registros [ true or false ]
			$this->btns_acoes['status_area_pesquisa']          = false; // Status da pesquisa [true or false]
			$this->btns_acoes['status_btn_filtros']            = false; // Status do botão de filtros [true or false]
			$this->btns_acoes['status_acao_pesquisa_avancada'] = false; // Status da ação de pesquisa avançada [true or false]
			...
			$this->btns_acoes['txt_btn_salvar']        = "Excluir Arquivos"; // Texto do btn salvar [true or false]
			...
			$this->btns_acoes['title_btn_salvar']        = "Excluir Arquivos (Alt+S)"; // Title do btn salvar [true or false]
			...
			$this->btns_acoes['class_icone_editar']       = "fa fa-archive"; // Classe do icone editar geral
			...
			$this->btns_acoes['class_icone_novo']         = "fa fa-archive"; // Classe do icone novo geral
			...
			$this->btns_acoes['class_icone_salvar']       = "fa fa-archive"; // Classe do icone do btn salvar
...
//***************************************************************************************************************************************************************
		function monto_conteudo_html_pagina($param){
			...
			if($param == 'campos_html_add_edd'){
			    ...
			    $this->funcoes->set_array(0,'id','1')->set_array(0,'value','Logs em txt');
				$this->funcoes->set_array(1,'id','2')->set_array(1,'value','Cache do Administrativo');
			    $this->funcoes->set_array(2,'id','3')->set_array(2,'value','Cache do Site');
			    ...
			    $mont_html->set_array(null,'label_for_texto','* O que você deseja excluir?');
			    $mont_html->set_array(null,'input_text_select','Selecione');
			    $mont_html->set_array(null,'input_name','opcoes_cache');
			    $mont_html->set_array(null,'input_id','opcoes_cache');
...
//***************************************************************************************************************************************************************
	function add_edd(){
        ...
        (string) $opcoes_p  = $this->funcoes->anti_injection($_POST['opcoes_cache']);
		...
		if(empty($id)){
			...
			//===========================================================
	        //EXECUTO LIMPO CACHE LOGS TXT
			if($opcoes_p == 1){

				//EXECUTO
				$dir     = $this->core->get_config('dir_raiz').'/libs/logs/';
				if($x = opendir($dir)){
				...
				$this->funcoes->set_array(null,'sucesso','Arquivos de logs em txt excluidos com sucesso!');
				echo json_encode($this->funcoes->get_array());
				$this->logs->setApp($this->dir_app)->setUrl($_SERVER['SERVER_NAME'].$_SERVER ['REQUEST_URI'])->setPagina(end(explode("/", $_SERVER['PHP_SELF'])))
		        ->setMensagem('Arquivos de logs em txt excluidos com sucesso!')->gravo_log();
		        exit();
			}

			//===========================================================
	        //EXECUTO LIMPO CACHE ADMINISTRATIVO
			if($opcoes_p == 2){

				//EXECUTO
				$dir     = $this->core->get_config('dir_raiz').'/adm/view/templates/default/cache/';
				if($x = opendir($dir)){
					while(false !== ($file = readdir($x))){
						if($file != '.' && $file != '..'){
							$path = $dir.”/”.$file;
				...
				//MENSAGEM DE SUCESSO E LIMPO CAMPOS
				$this->funcoes->set_array(null,'sucesso','Arquivos de cache do administrativo excluidos com sucesso!');
				echo json_encode($this->funcoes->get_array());
				$this->logs->setApp($this->dir_app)->setUrl($_SERVER['SERVER_NAME'].$_SERVER ['REQUEST_URI'])->setPagina(end(explode("/", $_SERVER['PHP_SELF'])))
		        ->setMensagem('Arquivos de cache do administrativo excluidos com sucesso!')->gravo_log();
		        exit();
			}

			//===========================================================
	        //EXECUTO LIMPO CACHE DO SITE
			if($opcoes_p == 3){

				//EXECUTO
				$dir     = $this->core->get_config('dir_raiz').'/web/view/templates/default/cache/';
				if($x = opendir($dir)){
					while(false !== ($file = readdir($x))){
						if($file != '.' && $file != '..'){
							$path = $dir.”/”.$file;
				...
				//MENSAGEM DE SUCESSO E LIMPO CAMPOS
				$this->funcoes->set_array(null,'sucesso','Arquivos de cache do site excluidos com sucesso!');
				echo json_encode($this->funcoes->get_array());
				$this->logs->setApp($this->dir_app)->setUrl($_SERVER['SERVER_NAME'].$_SERVER ['REQUEST_URI'])->setPagina(end(explode("/", $_SERVER['PHP_SELF'])))
		        ->setMensagem('Arquivos de cache do site excluidos com sucesso!')->gravo_log();
		        exit();
			}
