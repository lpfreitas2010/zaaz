...
class modulosController {
	...
	function __construct() {
		...
		$this->core->includeController('modulos',$this->dir_app); //incluo model ****-

		//INSTANCIO
		$this->funcoes           = new funcoes();     //Instancio Funções
		$this->model             = new modulosModel();  //Instancio Model ****-
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
            ...
            $this->url_pagina                    = "modulos"; //Url da página
            $this->nome_pagina_singular          = "Módulo"; // Nome da página singular
            $this->nome_pagina_plural            = "Módulos"; // Nome da página plural
            $this->btns_acoes['foco_campo_form'] = "modulo"; // Foco campo form add e edd
            ...
            $this->btns_acoes['title_campo_pesquisa'] = 'ID, Módulo ou Descrição'; // Parametros de pesquisa
            $this->btns_acoes['ordenar_selecionado']  = array("CADASTRO DECRESCENTE"); // Value padrão do campo Odernar
            ...
            $this->btns_acoes['mensagem_informativa'] = 'Faça o backup do banco de dados antes de inserir, editar ou excluir algum registro pois existem campos vinculados.'; // Mensagem informativa relacionada a tabela
...
//***************************************************************************************************************************************************************
		function monto_conteudo_html_pagina($param){
			...
			if($param == 'campos_html_add_edd'){
				...
				$mont_html->set_array(null,'input_name','modulo');
				$mont_html->set_array(null,'input_id','modulo');
				...
				$mont_html->set_array(null,'input_title','Digite o módulo');
				...
				$modulo = $mont_html->monto_html_input();

				//MONTO CAMPO INPUT TEXT
				$mont_html->set_array(null,'input_class_tamanho','col-lg-5 col-md-7 col-sm-12 col-xs-12'); //col-md-1 ao col-md-12 e outras classes
				$mont_html->set_array(null,'label_for_texto','* Descrição');
				$mont_html->set_array(null,'input_type','text'); //text, password
				$mont_html->set_array(null,'input_name','descricao');
				$mont_html->set_array(null,'input_id','descricao');
				$mont_html->set_array(null,'input_class',''); //[MASCARAS: (input-mask) (telefone) (money) (money2) ] [NÚMEROS use: (numeros)] [CALENDÁRIO: (date-picker) (time-picker) ]
				$mont_html->set_array(null,'input_placeholder','');
				$mont_html->set_array(null,'input_title','Digite a descrição');
				$mont_html->set_array(null,'title_help',''); //Ajuda Tags HTML ativadas
				$mont_html->set_array(null,'input_value','');
				$mont_html->set_array(null,'input_outros',''); // [MASCARA: (data-mask="000.000.000-00")]
				$mont_html->set_array(null,'input_disabled',''); //disabled
				$descricao = $mont_html->monto_html_input();
				...
				$mont_html->set_array(0,'conteudo',"{$modulo} {$descricao} "); //conteudo ***
			...
			if($param == 'listagem'){
				...
				$this->funcoes->set_array(1,'btn_texto','ID Módulo');
				...
				$this->funcoes->set_array(1,'btn_outros','param_pesquisa="ID Módulo: "');
				...
				$this->funcoes->set_array(3,'value',"Módulo CRESCENTE");
				$this->funcoes->set_array(4,'value',"Módulo DECRESCENTE");
				$this->funcoes->set_array(5,'value',"separator"); //separator
				$this->funcoes->set_array(6,'value',"CADASTRO CRESCENTE");
				$this->funcoes->set_array(7,'value',"CADASTRO DECRESCENTE");
				...
				$this->funcoes->set_array(1,'th_titulo','Módulo');
				$this->funcoes->set_array(1,'th_class','col-md-2');
				$this->funcoes->set_array(1,'th_outros','');
				$this->funcoes->set_array(2,'th_titulo','Descrição');
				$this->funcoes->set_array(2,'th_class','col-md-5');
				...
				$this->funcoes->set_array(4,'th_class','hidden-md text-center col-md-3 col-lg-2');
				$this->funcoes->set_array(4,'th_outros','');
				$this->funcoes->set_array(5,'th_titulo','<i class="fa fa-calendar-o"></i> Data Cadastro');
				$this->funcoes->set_array(5,'th_class','hidden-md text-center col-md-3 col-lg-2');
				...
				$mont_html->set_array(0,'url',$this->core->get_config('dir_raiz_http').$this->dir_app."/modulos_acoes_default/busca/ID Módulo: [bd:id]"); // url de redirecionamento $this->core->get_config('dir_raiz_http').$this->dir_app."/"
				$mont_html->set_array(0,'status_btn_detalhamento',$this->btns_acoes['status_btn_detalhamento']); // Status ação detalhamento true or false
				$mont_html->set_array(0,'status_btn_editar',$this->btns_acoes['status_btn_editar']); // Status ação editar true or false
				$mont_html->set_array(0,'status_btn_ativar',$this->btns_acoes['status_btn_ativar']); // Status ação ativar e desativar true or false
				$mont_html->set_array(0,'url_btn_ativar',$this->cmds['action_ativar']); // Action do btn ativar
				...
				$mont_html->set_array(1,'td',"[bd:modulo]"); // Coluna montada **
				...
				$mont_html->set_array(2,'td',"[bd:descricao]"); // Coluna montada **
				...
				$mont_html->set_array(3,'class',"text-center {$acao_linha2['class']}"); // Classes
				$mont_html->set_array(3,'outros'," {$acao_linha2['outros']}"); //Outros parametros do td
				$mont_html->set_array(4,'td',"{$modificado}"); // Coluna montada **
		...
		//SETO VALORES EM EDITAR
			...
			if(!empty($exec)){
				...
				for ($i=0; $i < count($exec) ; $i++) {
					...
					$this->funcoes->set_array(0,'campo_set_form','modulo');
					$this->funcoes->set_array(0,'valor_campo_set',$exec[$i]['modulo']);

					//CAMPO
					$this->funcoes->set_array(1,'tipo_set','input'); // input, textarea, checkbox, select
					$this->funcoes->set_array(1,'campo_set_form','descricao');
					$this->funcoes->set_array(1,'valor_campo_set',$exec[$i]['descricao']);
...
//***************************************************************************************************************************************************************
	function add_edd(){
		...
		$modulo = $this->funcoes->anti_injection($_POST['modulo']);
		$descricao = $this->funcoes->anti_injection($_POST['descricao']);

		//CONVERTO STRING PARA MAIUSCULO E MINUSCULO
		$modulo = $this->funcoes->conv_string($modulo,2);
		...
		if(empty($id)){
			...
			//===========================================================
			//VALIDO OS DADOS ****-
			if($this->model->retorn_campo_editar('modulo', $modulo) == $modulo){
				$modulo_u = true;
			}
			$this->funcoes->set('Módulo',"modulo", $modulo)->is_required()->min_length(4)->max_length(250)->is_unique($modulo_u);
			$this->funcoes->set('Descrição',"descricao", $descricao)->is_required()->min_length(2)->max_length(250);

			//===========================================================
	        //SETO OS DADOS ****-
			$this->model->setCampos('modulo',$modulo);
			$this->model->setCampos('descricao',$descricao);
		...
		//*************************************************************************************
			...
			$this->model->setId($id);
			if($this->model->retorn_campo_editar_val_id('modulo') != $modulo){
				if($this->model->retorn_campo_editar('modulo', $modulo) == $modulo){  $modulo_u = true;  }
				$this->funcoes->set('Módulo',"modulo", $modulo)->is_required()->min_length(4)->max_length(250)->is_unique($modulo_u);
			}else{
				$this->funcoes->set('Módulo',"modulo", $modulo)->is_required()->min_length(4)->max_length(250);
			}
			$this->funcoes->set('Descrição',"descricao", $descricao)->is_required()->min_length(2)->max_length(250);
