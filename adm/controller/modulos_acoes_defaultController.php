...
if (!empty($_GET['cmd']) || !empty($_POST['cmd'])) {
	$control = new modulos_acoes_defaultController();  // ****-
}

//CLASS ****-
class modulos_acoes_defaultController {
	...
	function __construct() {
		...
		$this->core->includeController('modulos_acoes_default',$this->dir_app); //incluo model ****-

		//INSTANCIO
		$this->funcoes           = new funcoes();     //Instancio Funções
		$this->model             = new modulos_acoes_defaultModel();  //Instancio Model ****-
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
            ...
            $this->url_pagina                    = "modulos_acoes_default"; //Url da página
            $this->nome_pagina_singular          = "Ação"; // Nome da página singular
            $this->nome_pagina_plural            = "Restrições Padrão"; // Nome da página plural
            $this->btns_acoes['foco_campo_form'] = "adm_usuario_modulo_id"; // Foco campo form add e edd
            ...
            $this->btns_acoes['title_campo_pesquisa'] = 'ID, Módulo ou Ação'; // Parametros de pesquisa
            $this->btns_acoes['ordenar_selecionado']  = array("MODULO CRESCENTE","ACAO CRESCENTE"); // Value padrão do campo Odernar
...
//***************************************************************************************************************************************************************
		function monto_conteudo_html_pagina($param){
			...
			if($param == 'campos_html_pesq_avancada'){
				...
				$mont_html->set_array(1,'conteudo'," {$area_descricao2} {$modulo} {$acoes} {$status}");
				$conteudo_montado = $mont_html->monto_html_forms();
			...
			if($param == 'listagem'){
				...
				$this->funcoes->set_array(3,'btn_texto','ID Módulo');
				...
				$this->funcoes->set_array(3,'btn_outros','param_pesquisa="ID Módulo: "');
				$this->funcoes->set_array(4,'btn_status',true);
				$this->funcoes->set_array(4,'btn_texto','ID Ação');
				...
				$this->funcoes->set_array(4,'btn_class','btn_param_pesquisa_focus'); // (btn_param_pesquisa) = seto e submeto a pesquisa - (btn_param_pesquisa_focus) = seto o valor e foco -
				$this->funcoes->set_array(4,'btn_class_icon','');
				$this->funcoes->set_array(4,'btn_outros','param_pesquisa="ID Ação: "');
				...
				$this->funcoes->set_array(9,'value',"MODULO CRESCENTE");
				$this->funcoes->set_array(10,'value',"MODULO DECRESCENTE");
				...
				$this->funcoes->set_array(1,'th_class',' col-md-2 col-lg-4');
				...
				$mont_html->set_array(0,'class',"load-elements text-center {$acao_linha['class']}"); // Classes
				$mont_html->set_array(0,'outros'," {$acao_linha['outros']}"); // Outros parametros do td
				$mont_html->set_array(1,'td'," [bd:modulo] "); // Coluna montada ****** texto, campo do banco [bd:campo] ou variavel smarty [sm:variavel]
				...
				$mont_html->set_array(2,'td'," {$acoes} "); // Coluna montada ****** texto, campo do banco [bd:campo] ou variavel smarty [sm:variavel]
		...
		//SETO VALORES EM EDITAR
			...
			if(!empty($exec)){
				...
				for ($i=0; $i < count($exec) ; $i++) {
					...
					$this->funcoes->set_array(0,'campo_set_form','adm_usuario_nivel_id');
					$this->funcoes->set_array(0,'valor_campo_set',$exec[$i]['adm_usuario_nivel_id']);
...
//***************************************************************************************************************************************************************
	function monto_campos_form_js(){
		...
		//MONTO COMBOBOX NIVEL
	    $this->funcoes->set_array(0,'tipo','combobox'); // Tipo
	    $this->funcoes->set_array(0,'tipo_combobox','nivel'); // tipo combobox
	    $this->funcoes->set_array(0,'id_combobox','adm_usuario_nivel'); // id do campo
	    $this->funcoes->set_array(0,'txt_plural_combobox','Níveis'); // Texto no plural
	    $this->funcoes->set_array(0,'txt_sing_combobox','Nível'); // Texto no singular
...
//***************************************************************************************************************************************************************
	function add_edd(){
		...
		(int) $adm_usuario_nivel            = $this->funcoes->anti_injection($_POST['adm_usuario_nivel']);
		...
		if(empty($id)){
			...
			//===========================================================
			//VALIDO OS DADOS ****-
      		$this->funcoes->set('Nível',"adm_usuario_nivel", $adm_usuario_nivel)->is_required()->is_num()->max_length(11);
			...
			if($this->model->retorn_campo_editar('adm_usuario_nivel_id', $adm_usuario_nivel) == $adm_usuario_nivel && $this->model->retorn_campo_editar('adm_usuario_modulo_id', $adm_usuario_modulo) == $adm_usuario_modulo && $this->model->retorn_campo_editar('adm_usuario_permissoes_acoes_id', $adm_usuario_permissoes_acoes) == $adm_usuario_permissoes_acoes ){
			...
			if($adm_usuario_permissoes_acoes == 17 && $opcoes == 0){
				...
				//===========================================================
				//SETO OS DADOS ****-
				$this->model->setCampos('adm_usuario_nivel_id',$adm_usuario_nivel);
		...
		//*************************************************************************************
			...
			//VALIDO SE TIVER CAMPO JA CADASTRADO
			$this->model->setId($id);
			if($this->model->retorn_campo_editar_val_id('adm_usuario_nivel_id') != $adm_usuario_nivel &&  $this->model->retorn_campo_editar_val_id('adm_usuario_modulo_id') != $adm_usuario_modulo && $this->model->retorn_campo_editar_val_id('adm_usuario_permissoes_acoes_id') != $adm_usuario_permissoes_acoes){
				if($this->model->retorn_campo_editar('adm_usuario_nivel_id', $adm_usuario_nivel) == $adm_usuario_nivel && $this->model->retorn_campo_editar('adm_usuario_modulo_id', $adm_usuario_modulo) == $adm_usuario_modulo && $this->model->retorn_campo_editar('adm_usuario_permissoes_acoes_id', $adm_usuario_permissoes_acoes) == $adm_usuario_permissoes_acoes ){
					$this->funcoes->set_array(null,'erro','Ação já cadastrada no banco de dados!');
					echo json_encode($this->funcoes->get_array());
					exit;
