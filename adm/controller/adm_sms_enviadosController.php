...
if (!empty($_GET['cmd']) || !empty($_POST['cmd'])) {
	$control = new adm_sms_enviadosController();  // ****-
}

//CLASS ****-
class adm_sms_enviadosController {
	...
	function __construct() {
		...
		$this->core->includeController('adm_sms_enviados',$this->dir_app); //incluo model ****-

		//INSTANCIO
		$this->funcoes           = new funcoes();     //Instancio Funções
		$this->model             = new adm_sms_enviadosModel();  //Instancio Model ****-
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
            ...
            $this->btns_acoes['title_campo_pesquisa'] = 'ID, Usuário,Telefone, Mensagem ou Status'; // Parametros de pesquisa
			...
			$this->btns_acoes['class_icone_listagem']     = "fa fa-mobile fa-lg"; // Classe do icone da listagem
...
//***************************************************************************************************************************************************************
	function view($parametro){
		...
		//===========================================================
		//OUTROS INCLUDES ****-
		/*$this->funcoes->set_array(0,'include',"graficos/grafico_total_sms_enviados.phtml")
					  ->set_array(0,'status_area',true) // controlo if de status
					  ->set_array(0,'col',"col-md-6") // col-md-
					  ->set_array(0,'height',"200") // height px
					  ->set_array(0,'tipo_grafico',"pizza_3d");  //tipos: anel, pizza, pizza_3d, barra_vertical, barra_horizontal
		$this->funcoes->set_array(1,'include',"graficos/grafico_total_logs_logoff_login.phtml")
					  ->set_array(1,'status_area',true) // controlo if de status
					  ->set_array(1,'col',"col-md-6") // col-md-
					  ->set_array(1,'height',"200") // height px
					  ->set_array(1,'tipo_grafico',"barra_vertical");  //tipos: anel, pizza, pizza_3d, barra_vertical, barra_horizontal
...
//***************************************************************************************************************************************************************
		function monto_conteudo_html_pagina($param){
			...
			if($param == 'listagem'){
				...
				$this->funcoes->set_array(7,'value',"IP CRESCENTE");
				$this->funcoes->set_array(8,'value',"IP DECRESCENTE");
				$this->funcoes->set_array(9,'value',"separator"); //separator
				$this->funcoes->set_array(10,'value',"SO CRESCENTE");
				$this->funcoes->set_array(11,'value',"SO DECRESCENTE");
				$this->funcoes->set_array(12,'value',"separator"); //separator
				$this->funcoes->set_array(13,'value',"NAVEGADOR CRESCENTE");
				$this->funcoes->set_array(14,'value',"NAVEGADOR DECRESCENTE");
				$this->funcoes->set_array(15,'value',"separator"); //separator
				$this->funcoes->set_array(16,'value',"IDIOMA CRESCENTE");
				$this->funcoes->set_array(17,'value',"IDIOMA DECRESCENTE");
				$this->funcoes->set_array(18,'value',"separator"); //separator
				$this->funcoes->set_array(19,'value',"CADASTRADO CRESCENTE");
				$this->funcoes->set_array(20,'value',"CADASTRADO DECRESCENTE");
				$this->funcoes->set_array(21,'value',"separator"); //separator
				$this->funcoes->set_array(22,'value',"MODIFICADO CRESCENTE");
				$this->funcoes->set_array(23,'value',"MODIFICADO DECRESCENTE");
				...
				$this->funcoes->set_array(1,'th_titulo','Telefone e Status');
				$this->funcoes->set_array(1,'th_class',' col-md-2 col-lg-2');
				$this->funcoes->set_array(1,'th_outros','');
				$this->funcoes->set_array(2,'th_titulo',' Mensagem');
				$this->funcoes->set_array(2,'th_class','col-md-3 col-lg-3');
				...
				$this->funcoes->set_array(4,'th_titulo','Outros');
				$this->funcoes->set_array(4,'th_class','hidden-md col-md-2 col-lg-3');
				$this->funcoes->set_array(4,'th_outros','');
				$this->funcoes->set_array(5,'th_titulo','<i class="fa fa-calendar-o"></i> Data Cadastro');
				$this->funcoes->set_array(5,'th_class','text-center col-md-1 col-lg-2 hidden-md');
				...
				$mont_html->set_array(0,'condicoes',"[bd:status] == '000' "); // condições do if - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
				...
				$mont_html->set_array(0,'condicoes',"[bd:status] == '000' "); // condições do if - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
				$mont_html->set_array(0,'cod_if'," <label class='label-success label'><i class='fa fa-check'></i> Enviado</label> "); // código do bloco if - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
				$mont_html->set_array(0,'cod_else',"<label class='label-danger label'><i class='fa fa-remove'></i> Erro no Envio</label>"); // código do bloco else se tiver - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
				$status = $mont_html->if_else_smarty();
				...
				$mont_html->set_array(1,'td'," <strong>TELEFONE:</strong> [bd:telefone]  <br /> <strong>STATUS:</strong> [bd:status] "); // Coluna montada **
				...
				$mont_html->set_array(2,'td'," [bd:mensagem]  "); // Coluna montada **
				...
				$mont_html->set_array(3,'td'," {$status}  "); // Coluna montada **
				...
				$mont_html->set_array(4,'td',"<strong>IP:</strong> [bd:IP]</br><strong>SISTEMA OP.:</strong> [bd:SO]</br><strong>NAVEGADOR:</strong> [bd:navegador]</br><strong>IDIOMA:</strong> [bd:idioma]"); // Coluna montada **
				$mont_html->set_array(4,'class',"hidden-md {$acao_linha['class']}"); // Classes
...
//***************************************************************************************************************************************************************
	function cmd_funcao_aberta(){
		...
		//===========================================================
		//RECEBO OS DADOS ****-
		$telefones          = $this->funcoes->anti_injection($_POST['sms_telefones']);
		(string) $mensagem  = $this->funcoes->anti_injection($_POST['sms_mensagem']);
		//$telefones = explode(",", $telefones);

        //TRATO CAMPO MULTIPLE
        $telefones = array_values($telefones); //reordeno array

		//===========================================================
		//VALIDO OS DADOS ****-
  		$this->funcoes->set('Mensagem',"sms_mensagem", $mensagem)->is_required()->max_length(140);
        ...
        if(count($telefones)==0){
            $this->funcoes->set_array(null,'erro','Insira os números de Telefone!'); //Mensagem
            echo json_encode($this->funcoes->get_array()); //Mostro na tela
            exit;
        }
		//MENSAGEM DE ERRO
		for ($i=0; $i <count($telefones) ; $i++) {
			if(!is_numeric($telefones[$i])){
				$this->funcoes->set_array(null,'erro','Número de telefone inválido! <br /> '.$telefones[$i].''); //Mensagem
				echo json_encode($this->funcoes->get_array()); //Mostro na tela
				exit;
			}
			if (strlen($telefones[$i]) < 10){
				$min = 1;
			}
			if (strlen($telefones[$i]) > 11){
				$max = 1;
			}
			if(!is_numeric($telefones[$i])){
				$numero = 1;
		...
		//MENSAGEM DE SUCESSO
		$this->funcoes->set_array(null,'sucesso','SMS enviados com sucesso!')->set_array(null,'limpo_campo','true'); // Mensagem de erro $id não é um array
		echo json_encode($this->funcoes->get_array());

		//GRAVO LOG
		$this->logs->setApp($this->dir_app)->setUrl($_SERVER['SERVER_NAME'].$_SERVER ['REQUEST_URI'])->setPagina(end(explode("/", $_SERVER['PHP_SELF'])))
		->setMensagem('SMS enviados pelo envio rápido')->gravo_log();
		exit();
	}
