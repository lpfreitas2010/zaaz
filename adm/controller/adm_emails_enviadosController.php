...
if (!empty($_GET['cmd']) || !empty($_POST['cmd'])) {
	$control = new adm_emails_enviadosController();  // ****-
}

//CLASS ****-
class adm_emails_enviadosController {
	...
	function __construct() {
		...
		$this->core->includeController('adm_emails_enviados',$this->dir_app); //incluo model ****-

		//INSTANCIO
		$this->funcoes           = new funcoes();     //Instancio Funções
		$this->model             = new adm_emails_enviadosModel();  //Instancio Model ****-
...
//***************************************************************************************************************************************************************
		function carrego_parametros(){
            ...
            $this->url_pagina                    = "adm_emails_enviados"; //Url da página
            $this->nome_pagina_singular          = "E-mail"; // Nome da página singular
            $this->nome_pagina_plural            = "Admin E-mails Enviados"; // Nome da página plural
            ...
            $this->btns_acoes['title_campo_pesquisa'] = 'Usuário, E-mail, Mensagem ou Status'; // Parametros de pesquisa
			...
			$this->btns_acoes['class_icone_listagem']     = "fa fa-envelope"; // Classe do icone da listagem
...
//***************************************************************************************************************************************************************
		function monto_conteudo_html_pagina($param){
			...
			if($param == 'campos_html_imprimir'){
				...
				$this->funcoes->set_array(1,'input_id','nome');
				...
				$this->funcoes->set_array(2,'input_id','acao');
				$this->funcoes->set_array(2,'input_name','campos_imprimir[]');
				$this->funcoes->set_array(2,'input_class','');
				$this->funcoes->set_array(2,'input_title','');
				$this->funcoes->set_array(2,'input_value','acao'); //value do campo
				$this->funcoes->set_array(2,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(2,'input_texto_checkbox','Ação'); //Texto
				$this->funcoes->set_array(3,'input_id','url');
				$this->funcoes->set_array(3,'input_name','campos_imprimir[]');
				$this->funcoes->set_array(3,'input_class','');
				$this->funcoes->set_array(3,'input_title','');
				$this->funcoes->set_array(3,'input_value','url'); //value do campo
				$this->funcoes->set_array(3,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(3,'input_texto_checkbox','Url'); //Texto
				$this->funcoes->set_array(4,'input_id','pagina');
				$this->funcoes->set_array(4,'input_name','campos_imprimir[]');
				$this->funcoes->set_array(4,'input_class','');
				$this->funcoes->set_array(4,'input_title','');
				$this->funcoes->set_array(4,'input_value','pagina'); //value do campo
				$this->funcoes->set_array(4,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(4,'input_texto_checkbox','Página'); //Texto
				$this->funcoes->set_array(5,'input_id','IP');
				$this->funcoes->set_array(5,'input_name','campos_imprimir[]');
				$this->funcoes->set_array(5,'input_class','');
				$this->funcoes->set_array(5,'input_title','');
				$this->funcoes->set_array(5,'input_value','IP'); //value do campo
				$this->funcoes->set_array(5,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(5,'input_texto_checkbox','IP'); //Texto
				$this->funcoes->set_array(6,'input_id','SO');
				$this->funcoes->set_array(6,'input_name','campos_imprimir[]');
				$this->funcoes->set_array(6,'input_class','');
				$this->funcoes->set_array(6,'input_title','');
				$this->funcoes->set_array(6,'input_value','SO'); //value do campo
				$this->funcoes->set_array(6,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(6,'input_texto_checkbox','SO'); //Texto
				$this->funcoes->set_array(7,'input_id','navegador');
				$this->funcoes->set_array(7,'input_name','campos_imprimir[]');
				$this->funcoes->set_array(7,'input_class','');
				$this->funcoes->set_array(7,'input_title','');
				$this->funcoes->set_array(7,'input_value','navegador'); //value do campo
				$this->funcoes->set_array(7,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(7,'input_texto_checkbox','Navegador'); //Texto
				$this->funcoes->set_array(8,'input_id','idioma');
				$this->funcoes->set_array(8,'input_name','campos_imprimir[]');
				$this->funcoes->set_array(8,'input_class','');
				$this->funcoes->set_array(8,'input_title','');
				$this->funcoes->set_array(8,'input_value','idioma'); //value do campo
				$this->funcoes->set_array(8,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(8,'input_texto_checkbox','Idioma'); //Texto
				$this->funcoes->set_array(9,'input_id','criado');
				...
				$this->funcoes->set_array(9,'input_value','criado'); //value do campo
				$this->funcoes->set_array(9,'input_checked','checked'); //checked ou ""
				$this->funcoes->set_array(9,'input_texto_checkbox','Data Cadastro'); //Texto
				$valores_checkbox = $this->funcoes->get_array();
			...
			if($param == 'campos_html_pesq_avancada'){
				...
				//CAMPO USUARIO
				$mont_html->set_array(null,'input_class_tamanho','col-lg-2 col-md-4 col-sm-12 col-xs-12'); //col-md-1 ao col-md-12
				...
				//CAMPO ACAO
				$mont_html->set_array(null,'input_class_tamanho','col-lg-3 col-md-4 col-sm-12 col-xs-12'); //col-md-1 ao col-md-12
				$mont_html->set_array(null,'label_for_texto','Ação ');
				$mont_html->set_array(null,'input_type','text');
				$mont_html->set_array(null,'input_name','pesq_avanc_acao');
				$mont_html->set_array(null,'input_id','pesq_avanc_acao');
				$mont_html->set_array(null,'input_class',''); //[MASCARAS: (input-mask) (telefone) (money) (money2) ] [NÚMEROS use: (numeros)] [CALENDÁRIO: (date-picker) (time-picker) ]
				$mont_html->set_array(null,'input_placeholder','');
				$mont_html->set_array(null,'input_title','Digite a ação');
				$mont_html->set_array(null,'title_help',''); //Ajuda Tags HTML ativadas
				$mont_html->set_array(null,'input_value',$valores_pesq_avancada['pesq_avanc_acao']);
				$mont_html->set_array(null,'input_outros',''); // [MASCARA: (data-mask="000.000.000-00")]
				$mont_html->set_array(null,'input_disabled',''); //disabled
				...
				//CAMPO URL
				$mont_html->set_array(null,'input_class_tamanho','col-lg-3 col-md-4 col-sm-12 col-xs-12'); //col-md-1 ao col-md-12
				$mont_html->set_array(null,'label_for_texto','URL ');
				$mont_html->set_array(null,'input_type','text');
				$mont_html->set_array(null,'input_name','pesq_avanc_url');
				$mont_html->set_array(null,'input_id','pesq_avanc_url');
				$mont_html->set_array(null,'input_class',''); //[MASCARAS: (input-mask) (telefone) (money) (money2) ] [NÚMEROS use: (numeros)] [CALENDÁRIO: (date-picker) (time-picker) ]
				$mont_html->set_array(null,'input_placeholder','');
				$mont_html->set_array(null,'input_title','Digite a URL');
				$mont_html->set_array(null,'title_help',''); //Ajuda Tags HTML ativadas
				$mont_html->set_array(null,'input_value',$valores_pesq_avancada['pesq_avanc_url']);
				$mont_html->set_array(null,'input_outros',''); // [MASCARA: (data-mask="000.000.000-00")]
				$mont_html->set_array(null,'input_disabled',''); //disabled
				$url = $mont_html->monto_html_input();

				//CAMPO PAGINA
				$mont_html->set_array(null,'input_class_tamanho','col-lg-2 col-md-3 col-sm-12 col-xs-12'); //col-md-1 ao col-md-12
				$mont_html->set_array(null,'label_for_texto','Página ');
				$mont_html->set_array(null,'input_type','text');
				$mont_html->set_array(null,'input_name','pesq_avanc_pagina');
				$mont_html->set_array(null,'input_id','pesq_avanc_pagina');
				$mont_html->set_array(null,'input_class',''); //[MASCARAS: (input-mask) (telefone) (money) (money2) ] [NÚMEROS use: (numeros)] [CALENDÁRIO: (date-picker) (time-picker) ]
				$mont_html->set_array(null,'input_placeholder','');
				$mont_html->set_array(null,'input_title','Digite a página');
				$mont_html->set_array(null,'title_help',''); //Ajuda Tags HTML ativadas
				$mont_html->set_array(null,'input_value',$valores_pesq_avancada['pesq_avanc_pagina']);
				$mont_html->set_array(null,'input_outros',''); // [MASCARA: (data-mask="000.000.000-00")]
				$mont_html->set_array(null,'input_disabled',''); //disabled
				$pagina = $mont_html->monto_html_input();
				...
				//===========================================================
				//MONTO CAMPOS
				$mont_html->set_array(0,'conteudo'," {$area_descricao} {$input_text} {$input_text2} {$select} {$area_descricao2} {$usuario} ");
			...
			if($param == 'listagem'){
				...
				$pesq_avanc8 = $this->funcoes->anti_injection($_POST['pesq_avanc_ip']); // Recebo dados
				$pesq_avanc9 = $this->funcoes->anti_injection($_POST['pesq_avanc_so']); // Recebo dados
				$pesq_avanc10 = $this->funcoes->anti_injection($_POST['pesq_avanc_navegador']); // Recebo dados
				$pesq_avanc11 = $this->funcoes->anti_injection($_POST['pesq_avanc_idioma']); // Recebo dados
				...
				if(!empty($pesq_avanc8)){ $pesq_avancada['pesq_avanc_ip'] = $pesq_avanc8; }
				if(!empty($pesq_avanc9)){ $pesq_avancada['pesq_avanc_so'] = $pesq_avanc9; }
				if(!empty($pesq_avanc10)){ $pesq_avancada['pesq_avanc_navegador'] = $pesq_avanc10; }
				if(!empty($pesq_avanc11)){ $pesq_avancada['pesq_avanc_idioma'] = $pesq_avanc11; }
				...
				$this->funcoes->set_array(6,'value',"IP CRESCENTE");
				$this->funcoes->set_array(7,'value',"IP DECRESCENTE");
				$this->funcoes->set_array(8,'value',"separator"); //separator
				$this->funcoes->set_array(9,'value',"SO CRESCENTE");
				$this->funcoes->set_array(10,'value',"SO DECRESCENTE");
				$this->funcoes->set_array(11,'value',"separator"); //separator
				$this->funcoes->set_array(12,'value',"NAVEGADOR CRESCENTE");
				$this->funcoes->set_array(13,'value',"NAVEGADOR DECRESCENTE");
				$this->funcoes->set_array(14,'value',"separator"); //separator
				$this->funcoes->set_array(15,'value',"IDIOMA CRESCENTE");
				$this->funcoes->set_array(16,'value',"IDIOMA DECRESCENTE");
				$this->funcoes->set_array(17,'value',"separator"); //separator
				$this->funcoes->set_array(18,'value',"CADASTRADO CRESCENTE");
				$this->funcoes->set_array(19,'value',"CADASTRADO DECRESCENTE");
				$this->funcoes->set_array(20,'value',"separator"); //separator
				$this->funcoes->set_array(21,'value',"MODIFICADO CRESCENTE");
				$this->funcoes->set_array(22,'value',"MODIFICADO DECRESCENTE");
				...
				$this->funcoes->set_array(1,'th_titulo','Ação');
				$this->funcoes->set_array(1,'th_class',' col-md-2 col-lg-3');
				...
				$this->funcoes->set_array(3,'th_titulo','Outros');
				$this->funcoes->set_array(3,'th_class','hidden-md col-md-2 col-lg-3');
				$this->funcoes->set_array(3,'th_outros','');
				$this->funcoes->set_array(4,'th_titulo','<i class="fa fa-calendar-o"></i> Data Cadastro');
				$this->funcoes->set_array(4,'th_class','text-center col-md-1 col-lg-2 hidden-md');
			    ...
			    $mont_html->set_array(0,'condicoes',"[bd:status] == 'Enviado' "); // condições do if - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
			    $mont_html->set_array(0,'cod_if'," <label class='label-success label'><i class='fa fa-check'></i> Enviado</label> "); // código do bloco if - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
				...
				//CONDIÇÕES IF ELSE SMARTY
			    $mont_html->set_array(0,'condicoes',"[bd:status] == 'Erro no envio' || [bd:status] == 'Erro ao enviar'"); // condições do if - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
			    $mont_html->set_array(0,'cod_if'," <label class='label-danger label'><i class='fa fa-remove'></i> Erro no Envio</label> "); // código do bloco if - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
			    $mont_html->set_array(0,'cod_else',""); // código do bloco else se tiver - aceita campo do banco [bd:campo] ou variavel smarty [sm:variavel]
				...
				//===========================================================
				//CONTEUDO MONTADO
				$mont_html->set_array(0,'td',"{$img_usuario} [bd:nome]"); // Coluna montada **
				...
				$mont_html->set_array(1,'td',"<strong>DE:</strong> [bd:email_re] <br /> <strong>PARA:</strong> [bd:email] <br /> <strong>MENSAGEM:</strong> ... "); // Coluna montada **
				...
				$mont_html->set_array(2,'td'," {$status}{$status1}  "); // Coluna montada **
				...
				$mont_html->set_array(3,'td',"<strong>IP:</strong> [bd:IP]</br><strong>SISTEMA OP.:</strong> [bd:SO]</br><strong>NAVEGADOR:</strong> [bd:navegador]</br><strong>IDIOMA:</strong> [bd:idioma]"); // Coluna montada **
				$mont_html->set_array(3,'class',"hidden-md {$acao_linha['class']}"); // Classes
...
//***************************************************************************************************************************************************************
	function monto_campos_form_js(){
	    ...
	    $this->funcoes->set_array(1,'tipo_autocomplete','acao'); // tipo autocomplete
	    $this->funcoes->set_array(1,'id_autocomplete','pesq_avanc_acao'); // id do campo

		$this->funcoes->set_array(2,'tipo','autocomplete'); // Tipo
	    $this->funcoes->set_array(2,'tipo_autocomplete','url'); // tipo autocomplete
	    $this->funcoes->set_array(2,'id_autocomplete','pesq_avanc_url'); // id do campo

		$this->funcoes->set_array(3,'tipo','autocomplete'); // Tipo
	    $this->funcoes->set_array(3,'tipo_autocomplete','pagina'); // tipo autocomplete
	    $this->funcoes->set_array(3,'id_autocomplete','pesq_avanc_pagina'); // id do campo

		$this->funcoes->set_array(4,'tipo','autocomplete'); // Tipo
	    $this->funcoes->set_array(4,'tipo_autocomplete','IP'); // tipo autocomplete
	    $this->funcoes->set_array(4,'id_autocomplete','pesq_avanc_ip'); // id do campo

		$this->funcoes->set_array(5,'tipo','autocomplete'); // Tipo
	    $this->funcoes->set_array(5,'tipo_autocomplete','SO'); // tipo autocomplete
	    $this->funcoes->set_array(5,'id_autocomplete','pesq_avanc_so'); // id do campo

		$this->funcoes->set_array(6,'tipo','autocomplete'); // Tipo
	    $this->funcoes->set_array(6,'tipo_autocomplete','navegador'); // tipo autocomplete
	    $this->funcoes->set_array(6,'id_autocomplete','pesq_avanc_navegador'); // id do campo

		$this->funcoes->set_array(7,'tipo','autocomplete'); // Tipo
	    $this->funcoes->set_array(7,'tipo_autocomplete','idioma'); // tipo autocomplete
	    $this->funcoes->set_array(7,'id_autocomplete','pesq_avanc_idioma'); // id do campo
...
//***************************************************************************************************************************************************************
	function add_edd(){
		...
		$nome = $this->funcoes->anti_injection($_POST['teste4'],'html');
		$nome1 = $this->funcoes->anti_injection($_POST['teste5'],'html');
		$select = $this->funcoes->anti_injection($_POST['teste_select']);
		$textarea = $this->funcoes->anti_injection($_POST['testee']);
		...
		if(empty($id)){
			...
			//===========================================================
			//VALIDO OS DADOS ****-
      		$this->funcoes->set('Nome',"teste4", $nome)->is_required()->min_length(2)->max_length(250);
			//$this->funcoes->set('Teste',"teste5", $nome1)->is_required()->min_length(2)->max_length(250)->is_num();
			//$this->funcoes->set('Teste',"teste_select", $select)->is_required();
			//$this->funcoes->set('Teste',"testee", $textarea)->is_required();

			//===========================================================
	        //SETO OS DADOS ****-
			$this->model->setCampos('url',$nome);
...
//***************************************************************************************************************************************************************
	function cmd_funcao_aberta(){
		...
		//===========================================================
		//RECEBO OS DADOS ****-
		$emails            = $this->funcoes->anti_injection($_POST['emails_email']);
		(string) $assunto  = $this->funcoes->anti_injection($_POST['email_assunto']);
		(string) $mensagem = $this->funcoes->anti_injection($_POST['email_mensagem'],'html');
		//$emails = explode(",", $emails);
		$this->core->includeHelper('upload');
		$upload = new upload();
		$arquivos = $_FILES['arquivos_email'];

		//TRATO CAMPO MULTIPLE
        $emails = array_values($emails); //reordeno array

		//===========================================================
		//VALIDO OS DADOS ****-
		$this->funcoes->set('Assunto',"email_assunto", $assunto)->max_length(50);
		$this->funcoes->set('Mensagem',"email_mensagem", $mensagem)->is_required()->max_length(10000);
		...
		if(count($emails)==0){
			$this->funcoes->set_array(null,'erro','Insira os e-mails!'); //Mensagem
			echo json_encode($this->funcoes->get_array()); //Mostro na tela
			exit;
		}
		//MENSAGEM DE ERRO
		for ($i=0; $i <count($emails) ; $i++) {
			if (filter_var($emails[$i], FILTER_VALIDATE_EMAIL) === false) {
				$this->funcoes->set_array(null,'erro','E-mail inválido! <br />'.$emails[$i].''); //Mensagem
				echo json_encode($this->funcoes->get_array()); //Mostro na tela
				exit();
			}
		}
		...
		$this->email->setArquivo($arquivos); //Arquivo de anexo
		$this->email->setAssunto($assunto); //Assunto
		$this->email->setConteudo($mensagem.'<br />'.$this->model->retorno_assinatura_usuario()); //Conteúdo
		...
		if($exec_email != true){
			...
			//GRAVO LOG
			$this->logs->setApp($this->dir_app)->setUrl($_SERVER['SERVER_NAME'].$_SERVER ['REQUEST_URI'])->setPagina(end(explode("/", $_SERVER['PHP_SELF'])))
			->setMensagem('E-mails enviados pelo envio rápido')->gravo_log();
			exit();
		}
...
//***************************************************************************************************************************************************************
//FUNÇÃO RETORNA DADOS DE GRAFICO
//***************************************************************************************************************************************************************
